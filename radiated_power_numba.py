...
def rpa_radiated_power(
    ...
    for m in range(num_el_x):
        for n in range(num_el_y):
            ...
            electric_field_preallocated += weights[element] * np.exp(exponent)
    radiated_power = np.power(np.abs(electric_field_preallocated), 2)
    return radiated_power


@jit
def ula_radiated_power(
    complex_weights,
    electric_field_preallocated,
    num_elements,
    wavenumber,
    ...
    for element in range(num_elements):
        exponent = phases[element] + (1j * wavenumber * (element * spacing * u_grid))
        electric_field_preallocated += weights[element] * np.exp(exponent)
    radiated_power = np.power(np.abs(electric_field_preallocated), 2)
    return radiated_power
...
def ca_radiated_power(
    ...
    for element in range(num_elements):
        ...
        exponent = phases[element] + 1j * wavenumber * (
            radius * (np.sin(element_angle) * u_grid + np.cos(element_angle) * v_grid)
        )
        electric_field_preallocated += weights[element] * np.exp(exponent)
    radiated_power = np.power(np.abs(electric_field_preallocated), 2)
    return radiated_power
